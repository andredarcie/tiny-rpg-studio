...
type VariableServiceManager = ConstructorParameters<typeof EditorVariableService>[0];
type VariableServiceManagerFixture = ReturnType<typeof makeManager>;

function asVariableServiceManager(mgr: VariableServiceManagerFixture): VariableServiceManager {
  return mgr as unknown as VariableServiceManager;
}
...
describe('EditorVariableService', () => {
  ...
  it('returns early when variableId is empty', () => {
    ...
    const svc = new EditorVariableService(asVariableServiceManager(mgr));
    svc.toggle('');
